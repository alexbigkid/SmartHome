...
if [ "$ERROR_CODE" -eq $ERROR_CODE_SUCCESS ]; then
    ...
    mysql --defaults-file=mysqlDbConnectionLocal.conf abkcompa_shDev -e 'DROP TABLE `Company`;'
    mysql --defaults-file=mysqlDbConnectionLocal.conf abkcompa_shDev -e 'DROP TABLE `FootballWorldCup`;'
    mysql --defaults-file=mysqlDbConnectionLocal.conf abkcompa_shDev -e 'DROP TABLE `State`;'
    mysql --defaults-file=mysqlDbConnectionLocal.conf abkcompa_shDev -e 'DROP TABLE `Country`;'
    mysql --defaults-file=mysqlDbConnectionLocal.conf abkcompa_shDev -e 'DELETE FROM `flyway_schema_history` WHERE `installed_rank` <> 1;'
    #mysql --defaults-file=mysqlDbConnectionLocal.conf abkcompa_shDev -e 'DROP TABLE `Employees`; DROP TABLE `Company`; DROP TABLE `FootballWorldCup`; DROP TABLE `State`; DROP TABLE `Country`; DELETE FROM `flyway_schema_history` WHERE `installed_rank` <> 1;'
